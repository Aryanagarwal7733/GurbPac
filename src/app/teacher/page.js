...
export default function TeacherDashboard() {
  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const contents = await contentService.getTeacherContent(user.id);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400">Here's a summary of your uploaded content.</p>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card className="dark:bg-gray-950 dark:border-gray-800 transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Uploaded</CardTitle>
